...
export default function App() {
  ...
  const isGameMode = location.pathname.startsWith('/arcade') || location.pathname.startsWith('/game');
        ...
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-3">
             <button 
              ...
              className={`px-4 py-2 rounded-full text-sm font-bold transition-all whitespace-nowrap ${isGameMode ? activeThemeClass : 'text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800'}`}
            >
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        <Routes>
          ...
          <Route path="/arcade" element={
            <GameMode settings={settings} setHeaderConfig={setHeaderConfig} updateSettings={updateSettings} />
          } />

          <Route path="/game/:variant" element={
            <GameMode settings={settings} setHeaderConfig={setHeaderConfig} updateSettings={updateSettings} />
          } />
